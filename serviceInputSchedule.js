...
function inputScheduleToSheet(r, tasks, backlogTaskHash) {
  ...
  tasks.forEach((task, index) => {
    ...
    sheet.getRange(row, 3).setValue(task.name);    // C列にタスク名
    sheet.getRange(row, 8).setValue(doneValue);    // H列に完了ステータス
    sheet.getRange(row, 9).setValue(task.asp);     // I列にASP
    sheet.getRange(row, 10).setValue(task.duration); // J列に所要時間
    sheet.getRange(row, 12).setValue(task.startTime); // L列に開始時刻
    sheet.getRange(row, 13).setValue(task.endTime);   // M列に終了時刻
  });
}
